...
class BarnTest : FunSpec({
    ...
    context("Barnetillegg med wrapped funksjon") {
        test("Med 0 barn") {
            val respons = Respons(0.0, PersonInfo(0.0, 0.0, 0.0, 0, 0.0, aar, false))
            respons.barnetillegg()
            respons.logs.size shouldBe 0
        }
        test("med 1 barn") {
            val respons = Respons(100000.0, PersonInfo(0.0, 0.0, 0.0, 1, 0.0, aar, false))
            respons.barnetillegg()
        ...
        test("8 barn 2g") {
            val respons = Respons(222954.0 * 0.66, PersonInfo(0.0, 0.0, 0.0, 8, 0.0, aar, false))
            respons.barnetillegg()
